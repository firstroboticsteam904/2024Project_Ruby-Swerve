...
public class LimelightTracking extends Command {
  ...
  private final Supplier<Double> xSpeedFunc, ySpeedFunc;
  private final double trackingRotationFunc;
  private final Boolean fieldOrientedFunc;
  private final SlewRateLimiter xRateLimiter, yRateLimiter, rotationRateLimiter;

  /** Creates a new TeleopDrive. */
  public LimelightTracking(DriveSubsystem swerveDrivetrain, Supplier<Double> xSpeed,
  Supplier<Double> ySpeed, double trackingRotation, boolean FieldRelative) {
    // Use addRequirements() here to declare subsystem dependencies.
    swerveDrive = swerveDrivetrain;
    xSpeedFunc = xSpeed;
    ySpeedFunc = ySpeed;
    trackingRotationFunc = trackingRotation;
    fieldOrientedFunc = FieldRelative;
  ...
  public void execute() {
     ...
     PIDController Seeza_LimelightPID = new PIDController(0.5, 0, 0.01);
    ...
    swerveDrive.limelightTracking(
      -ForwardX, 
      -StrafeY, 
      swerveDrive.limelightCenter(), 
      false);

      if(fieldOrientedFunc == false){
